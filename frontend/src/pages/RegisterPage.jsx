...
// ─── API register ──────────────────────────────────────────────────────────
const apiRegister = async ({ fullName, email, username, password, perfil }) => {
  const base = import.meta.env.VITE_API_URL || "http://localhost:3001";
  const res  = await fetch(`${base}/api/auth/register`, {
    method:  "POST",
    headers: { "Content-Type": "application/json" },
    body:    JSON.stringify({ fullName, email, username, password, role: perfil }),
  });
  const data = await res.json();
  if (!res.ok) throw new Error(data.code || data.message || "register_error");
  return data;
};
