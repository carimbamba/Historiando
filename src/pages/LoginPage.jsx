...
/**
 * mockLogin — REMOVA quando integrar com a API real.
 * Credenciais de teste:
 *   professor@escola.edu.br  → Dashboard  (/)
 *   coord@escola.edu.br      → Analytics  (/analytics)
 *   aluno@escola.edu.br      → Conteúdo   (/conteudo)
 *   senha: qualquer string com 6+ caracteres
 */
const mockLogin = async (email, password) => {
  await new Promise((r) => setTimeout(r, 900));
  const MOCK = {
    "professor@escola.edu.br": { token: "mock-jwt-prof",  role: "professor",   name: "Prof. Marcos P.", expiresIn: 86400 },
    "coord@escola.edu.br":     { token: "mock-jwt-coord", role: "coordenador", name: "Coord. Patrícia", expiresIn: 86400 },
    "aluno@escola.edu.br":     { token: "mock-jwt-aluno", role: "aluno",       name: "João Pedro",      expiresIn: 86400 },
  };
...
export default function LoginPage() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      /*
       * ── INTEGRAÇÃO tRPC ──────────────────────────────────────────
       * const result = await trpc.auth.login.mutateAsync({
       ...
       */
      const result = await mockLogin(data.email, data.password); // ← remover em prod
