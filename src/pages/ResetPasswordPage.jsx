...
// ─── Validação de token (mock) ─────────────────────────────────────────────
// Em produção: o backend valida o JWT (assinatura + expiração + uso único).
// Aqui simulamos 3 cenários possíveis no frontend.
const MOCK_VALID_TOKENS = {
  "mock-valid-token-123": { valid: true,  expired: false, used: false, email: "pr***@escola.edu.br" },
  "mock-expired-token":   { valid: true,  expired: true,  used: false, email: "pr***@escola.edu.br" },
  "mock-used-token":      { valid: true,  expired: false, used: true,  email: "pr***@escola.edu.br" },
};

const mockValidateToken = async (token) => {
  await new Promise((r) => setTimeout(r, 900));
  const result = MOCK_VALID_TOKENS[token];
  if (!result)          throw new Error("invalid");
  if (result.expired)   throw new Error("expired");
  if (result.used)      throw new Error("used");
  return { email: result.email };
};

const mockResetPassword = async (token, password) => {
  await new Promise((r) => setTimeout(r, 1100));
  if (!MOCK_VALID_TOKENS[token]) throw new Error("invalid");
  // Invalida o token (mock)
  MOCK_VALID_TOKENS[token] = { ...MOCK_VALID_TOKENS[token], used: true };
  return { ok: true };
};
...
export default function ResetPasswordPage() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      /*
       * ── INTEGRAÇÃO tRPC ──────────────────────────────────────────────
       * await trpc.auth.resetPassword.mutateAsync({
       *   token:       token,
       ...
       *   if (err.code === "TOKEN_EXPIRED") throw new Error("expired");
       *   if (err.code === "TOKEN_USED")    throw new Error("used");
       *   throw new Error("invalid");
       * }
       */
      await mockResetPassword(token, data.password); // ← remover em prod
